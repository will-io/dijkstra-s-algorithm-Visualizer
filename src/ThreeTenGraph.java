...
/**
 * The Class ThreeTenGraph represents a directed graph
 *
...
class ThreeTenGraph<V, E> implements Graph<V, E>, DirectedGraph<V, E> {
	...
	private HashMap<V, ArrayList<V>> neighbors = new HashMap<>();
	private HashMap<V, ArrayList<E>> edges = new HashMap<>();
	...
	public Collection<E> getEdges() {
		...
		Collection<E> all = new LinkedList<>();
		for (Entry<V, ArrayList<E>> e : edges.entrySet()) {
	...
	public Collection<V> getVertices() {
		...
		for (Entry<V, ArrayList<V>> entry : neighbors.entrySet()) {
	...
	public boolean containsVertex(V vertex) {
		...
		for (Entry<V, ArrayList<V>> entry : neighbors.entrySet()) {
			if (entry.getKey().equals(vertex) || entry.getValue().contains(vertex))
				return true;
		}
	...
	public Collection<E> getInEdges(V vertex) {
		Collection<E> incoming = new HashSet<>();
		for (Entry<V, ArrayList<V>> entry : neighbors.entrySet()) {
			int index = entry.getValue().indexOf(vertex);
			if (index != -1) {
				incoming.add(edges.get(entry.getKey()).get(index));
			}
		}
	...
	public Collection<V> getPredecessors(V vertex) {
		Collection<V> predecessors = new HashSet<>();
		for (Entry<V, ArrayList<V>> entry : neighbors.entrySet()) {
	...
	public boolean isPredecessor(V v1, V v2) {
		for (Entry<V, ArrayList<V>> entry : neighbors.entrySet()) {
			if (entry.getValue().contains(v1) && entry.getKey().equals(v2))
				return true;
		}
	 ...
	 *
	 * @param directed_edge the directed edge
	...
	public V getSource(E directed_edge) {
		...
		for (Entry<V, ArrayList<E>> e : edges.entrySet()) {
			int index = e.getValue().indexOf(directed_edge);
			if (index != -1)
				return e.getKey();
		}
	...
	public V getDest(E directed_edge) {
		...
		for (Entry<V, ArrayList<E>> e : edges.entrySet()) {
			int index = e.getValue().indexOf(directed_edge);
			if (index != -1)
				return neighbors.get(e.getKey()).get(index);
		}
	...
	public Collection<V> getNeighbors(V vertex) {
		...
		result.addAll(neighbors.get(vertex));
	...
	public Collection<E> getIncidentEdges(V vertex) {
		...
		result.addAll(getInEdges(vertex));
	...
	public Collection<V> getIncidentVertices(E edge) {
		...
		Collection<V> result = new HashSet<>();
		result.add(getSource(edge));
		result.add(getDest(edge));
		return result;
	}
	...
	public E findEdge(V v1, V v2) {
		if (!containsVertex(v1) || !containsVertex(v2) || !neighbors.containsKey(v1))
			return null;

		int index = neighbors.get(v1).indexOf(v2);
		return index == -1 ? null : edges.get(v1).get(index);
	}
	...
	public boolean addEdge(E e, V v1, V v2) {
		...
		System.out.println(v1 + " " + v2 + " " + e);

		if (!neighbors.containsKey(v1)) {
			neighbors.put(v1, new ArrayList<>());
	...
	public boolean removeEdge(E edge) {
		...
		for (Entry<V, ArrayList<E>> entry : edges.entrySet()) {
			int index = entry.getValue().indexOf(edge);
			if (index != -1) {
				entry.getValue().remove(index);
				neighbors.get(entry.getKey()).remove(index);
				return true;
			}
		}
	...
	public boolean removeVertex(V vertex) {
		...
		for (Entry<V, ArrayList<V>> entry : neighbors.entrySet()) {
			int index;
			while ((index = entry.getValue().indexOf(vertex)) != -1) {
				entry.getValue().remove(index);
				edges.get(entry.getKey()).remove(index);//edges.get(vertex).remove(index);//needs fix
				removed = true;
			}
		}
